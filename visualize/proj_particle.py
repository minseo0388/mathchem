# proj_particle.py
# Visualization for Particle in a Box models.

import matplotlib.pyplot as plt
import numpy as np
from mpl_toolkits.mplot3d import Axes3D

def plot_wavefunction_1d(particle, n: int, x: np.ndarray):
    """
    Plot the wavefunction of a 1D particle in a box.

    Parameters:
        particle: ParticleInABox1D instance.
        n: Quantum number (int).
...
def plot_wavefunction_2d(particle, nx: int, ny: int, x: np.ndarray, y: np.ndarray):
    """
    Plot the wavefunction of a 2D particle in a box.

    Parameters:
        particle: ParticleInABox2D instance.
        nx: Quantum number in x direction (int).
        ny: Quantum number in y direction (int).
